...
public class ExcavatorToolItem extends BaseToolItem implements PolymerItem, PolymerKeepModel, PolymerClientDecoded {
    ...
    private static Settings createSettings(ToolMaterial baseMaterial) {
        var settings = new Settings()
                ...
                .component(DataComponentTypes.LORE, new LoreComponent(List.of(Text.translatable("item.moretools.excavator.tooltip").formatted(Formatting.GOLD))));
    ...
    public ExcavatorToolItem(ShovelItem base, ToolMaterial baseMaterial) {
        super(base, ExcavatorToolItem.createSettings(baseMaterial), Identifier.of(MoreTools.MOD_ID, Registries.ITEM.getId(base).getPath().replace("shovel", "excavator")), baseMaterial, BlockTags.SHOVEL_MINEABLE);
        this.baseItem = base;
    }
